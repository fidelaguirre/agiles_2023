...
import { getBanda, listEventos } from "../graphql/queries";
import { updateBanda, updateEvento } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function BandaUpdateForm(props) {
  const {
    id: idProp,
    banda: bandaModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = bandaRecord
      ? {
          ...initialValues,
          ...bandaRecord,
          integrantes: linkedIntegrantes,
          Eventos: linkedEventos,
        }
      : initialValues;
    setName(cleanValues.name);
    setIntegrantes(cleanValues.integrantes ?? []);
    setCurrentIntegrantesValue(undefined);
    setCurrentIntegrantesDisplayValue("");
    setEventos(cleanValues.Eventos ?? []);
    setCurrentEventosValue(undefined);
    setCurrentEventosDisplayValue("");
    setErrors({});
  };
  const [bandaRecord, setBandaRecord] = React.useState(bandaModelProp);
  const [linkedIntegrantes, setLinkedIntegrantes] = React.useState([]);
  const canUnlinkIntegrantes = true;
  const [linkedEventos, setLinkedEventos] = React.useState([]);
  const canUnlinkEventos = true;
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ...
        : bandaModelProp;
      const linkedIntegrantes = record?.integrantes?.items ?? [];
      setLinkedIntegrantes(linkedIntegrantes);
      const linkedEventos = record?.Eventos?.items ?? [];
      setLinkedEventos(linkedEventos);
      setBandaRecord(record);
    };
    queryData();
  }, [idProp, bandaModelProp]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          const promises = [];
          const integrantesToLink = [];
          const integrantesToUnLink = [];
          const integrantesSet = new Set();
          const linkedIntegrantesSet = new Set();
          integrantes.forEach((r) =>
            integrantesSet.add(getIDValue.integrantes?.(r))
          );
          linkedIntegrantes.forEach((r) =>
            linkedIntegrantesSet.add(getIDValue.integrantes?.(r))
          );
          linkedIntegrantes.forEach((r) => {
            if (!integrantesSet.has(getIDValue.integrantes?.(r))) {
              integrantesToUnLink.push(r);
            }
          });
          integrantes.forEach((r) => {
            if (!linkedIntegrantesSet.has(getIDValue.integrantes?.(r))) {
              integrantesToLink.push(r);
            }
          });
          integrantesToUnLink.forEach((original) => {
            if (!canUnlinkIntegrantes) {
              throw Error(
                `Evento ${original.id} cannot be unlinked from Banda because undefined is a required field.`
              );
            }
            promises.push(
              API.graphql({
          ...
          const eventosToLink = [];
          const eventosToUnLink = [];
          const eventosSet = new Set();
          const linkedEventosSet = new Set();
          Eventos.forEach((r) => eventosSet.add(getIDValue.Eventos?.(r)));
          linkedEventos.forEach((r) =>
            linkedEventosSet.add(getIDValue.Eventos?.(r))
          );
          linkedEventos.forEach((r) => {
            if (!eventosSet.has(getIDValue.Eventos?.(r))) {
              eventosToUnLink.push(r);
            }
          });
          Eventos.forEach((r) => {
            if (!linkedEventosSet.has(getIDValue.Eventos?.(r))) {
              eventosToLink.push(r);
            }
          });
          eventosToUnLink.forEach((original) => {
            if (!canUnlinkEventos) {
              throw Error(
                `Evento ${original.id} cannot be unlinked from Banda because undefined is a required field.`
              );
      ...
      {...getOverrideProps(overrides, "BandaUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || bandaModelProp)}
